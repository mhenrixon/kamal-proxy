...
func newDeployCommand() *deployCommand {
	...
	deployCommand.cmd.Flags().StringSliceVar(&deployCommand.args.ServiceOptions.Cache.VaryHeaders, "cache-vary-header", nil, "Request header(s) the --cache key is computed over, e.g. Accept-Language. A response whose Vary names anything not listed here is passed through uncached rather than risking one client's copy answering another's request. Accept-Encoding needs no entry: the cache stores what the target produced and --compress encodes it per client")

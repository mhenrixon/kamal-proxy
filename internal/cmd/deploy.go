...
func newDeployCommand() *deployCommand {
	...
	deployCommand.cmd.Flags().StringVar(&deployCommand.args.ServiceOptions.RedirectsSource, "redirects-source", "", "Fetch a host-scoped redirect map from this endpoint (path resolved against the service, or absolute URL). The app publishes {\"hosts\": {...}} entries -- per-host redirect_to, path rules and trailing-slash policy -- and the proxy answers matching requests itself. Both tokens live in the PROXY's environment, not this client's: polls send KAMAL_PROXY_REDIRECTS_TOKEN as a bearer token when the proxy has it set, and POST /.kamal-proxy/redirects/refresh nudges an immediate re-poll when authenticated with the proxy's KAMAL_PROXY_REFRESH_TOKEN")
